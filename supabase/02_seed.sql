...
begin
  ...
  -- 7. Demo challenges
  delete from public.club_challenges where title like 'DEMO · %';

  insert into public.club_challenges (club_id, author_id, title, goal_type, goal_value, start_at, end_at)
  values
    (v_club_sh, v_owner, 'DEMO · May 500K Challenge', 'distance_total', 500000,
     date_trunc('month', v_now), date_trunc('month', v_now) + interval '1 month'),
    (v_club_sh, v_owner, 'DEMO · Get 5 Members Running', 'runners_count', 5,
