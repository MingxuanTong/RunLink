...
drop policy if exists reg_read_clubmates           on public.registrations;
...
-- Read: the user themselves, OR any club member (for crew strip visibility)
create policy reg_read_clubmates on public.registrations
  for select to authenticated
  using (
    user_id = auth.uid()
    or exists (
...
-- club_challenges
-- =====================================================================
drop policy if exists challenges_read_member     on public.club_challenges;
drop policy if exists challenges_insert_organizer on public.club_challenges;
drop policy if exists challenges_delete_organizer on public.club_challenges;

create policy challenges_read_member on public.club_challenges
  for select to authenticated
  using (public.is_club_member(club_id));

create policy challenges_insert_organizer on public.club_challenges
  for insert to authenticated
  with check (
    author_id = auth.uid()
    and public.is_club_organizer(club_id)
  );

create policy challenges_delete_organizer on public.club_challenges
  for delete to authenticated
  using (public.is_club_organizer(club_id));
