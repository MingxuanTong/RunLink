...
-- =====================================================================
-- 8. club_challenges (collective goals)
-- =====================================================================
create table if not exists public.club_challenges (
  ...
  title       text not null,
  goal_type   text not null default 'distance_total'
              check (goal_type in ('distance_total','runners_count')),
  ...
  created_at  timestamptz not null default now(),
  updated_at  timestamptz not null default now(),
  constraint chk_challenge_dates check (end_at > start_at)
);

drop trigger if exists trg_challenges_updated_at on public.club_challenges;
create trigger trg_challenges_updated_at
  before update on public.club_challenges
  for each row execute function public.tg_set_updated_at();

create index if not exists idx_challenges_club on public.club_challenges(club_id);
create index if not exists idx_challenges_active on public.club_challenges(club_id, end_at)
...
-- =====================================================================
-- Helper view: challenge progress (auto-computed from runs)
-- =====================================================================
create or replace view public.v_challenge_progress as
select
  ...
  ch.achieved_at,
  case ch.goal_type
    when 'distance_total' then coalesce(agg.total_distance, 0)
    when 'runners_count'  then coalesce(agg.runner_count, 0)
  end::bigint as current_value,
  ch.end_at < now() as is_expired
from public.club_challenges ch
left join lateral (
  select
    sum(r.distance_m)::bigint as total_distance,
    count(distinct r.user_id)::int as runner_count
  from public.runs r
  join public.club_members cm on cm.user_id = r.user_id and cm.club_id = ch.club_id
  where r.started_at >= ch.start_at
    and r.started_at <  ch.end_at
) agg on true;
